...
such as shared state with \texttt{IORef}'s and message passing with channels.
...
The string value produced gives an error message reporting the
...
mutual exclusion, are nested.  \texttt{MVar}'s (mutable variables) in Haskell can be
...
using the existing \texttt{Either} monad transformer. This propagates a
\texttt{Left String} value which contains the error message when an ownership
typing violation occurs.
Future work could instead use a structured error type in place of the string.
