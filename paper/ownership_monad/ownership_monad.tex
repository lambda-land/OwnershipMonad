...
reference must have a \texttt{Writable} flag.
...
ID's to entries. It is useful to know that if an ORef has been dropped and
if it never existed. % TODO add more to this. Explain the different error?
...
set to the \texttt{Nothing} case since it will no longer be used.
...
To perfom a thread ID check the \texttt{inThreadAndReadable} function
...
resource in a borrow operation in a way that may mutate the value stored in the
owned reference.
