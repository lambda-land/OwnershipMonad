...
\abstract{
  ...
  This concept builds off of Affine types and allows the user to safely mutate
  resources by restricting resource ownership to at most one variable binding
  at a time.
  In large part because of this, the Rust language is able to give a user safe
  low level control over resources while forgoing garbage collection and the
  overhead of a runtime.
  In Haskell data is immutable and many of the problems addressed by
  ownership in Rust are resolved in Haskell with referential transparency.
  This project report introduces the Ownership Monad, a monad which implements
  aspects of the ownership system as a library in Haskell.
  This will demonstrate many of rules enforced by the ownership system in Rust
  in order to better understand the ownership based method of tracking
  ...
  % Owned Channels, an abstraction which sends ownership over an underlying
  % resource to a thread, in addition to the resource itself.
}
